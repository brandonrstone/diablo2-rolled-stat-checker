...
export default function App() {
  ...
  const filteredRunewords = useMemo(() => {
    ...
    return Runewords.filter((rw: RunewordType) => includesAllTokens(runewordHaystack(rw), tokens));
  }, [tokens]);

  const filteredSetItems = useMemo(() => {
    if (!tokens.length) return [];
    return SetItems.filter((si: SetItemType) => includesAllTokens(setItemHaystack(si), tokens));
  }, [tokens]);

  const filteredUniqueItems = useMemo(() => {
    if (!tokens.length) return [];
    return UniqueItems.filter((ui: UniqueItemType) => includesAllTokens(uniqueItemHaystack(ui), tokens));
  }, [tokens]);
      ...
      <div className="fixed top-0 left-0 right-0 z-20">
        ...
        >
          <div className={`relative bottom-2 flex flex-col items-center ${blurred && 'text-white/90'}`}>
            ...
            <span className="text-ui-gold font-system-ui [font-size:clamp(0.95rem,0.8rem+0.4vw,1.1rem)]">
              Rolled Stat Checker v1.3
            </span>
            ...
            <label className="w-full">
              <div
                className="
                  ...
                  shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_1px_8px_rgba(0,0,0,0.25)]
                  hover:bg-white/7 transition
                  focus-within:ring-2 focus-within:ring-yellow-700/40
                  px-4
                "
              >
                <input
                  className="
                    ...
                    text-base font-sans text-white/95 placeholder:text-white/60
                    selection:bg-yellow-700/40 selection:text-white
                  "
                  type="text"
            ...
            {filteredRunewords.length > 0 && (
              <>
                ...
                {filteredRunewords.map((runeword: RunewordType) => (
                  <Runeword key={runeword.id} runeword={runeword} />
                ))}
              </>
            )}
          </>
...
function runewordHaystack(runeword: RunewordType): string {
  ...
  if (Array.isArray(runeword.stats) && runeword.stats.length) {
    parts.push(...(runeword.stats.map(s => s.code).filter(Boolean) as string[]));
  } else {
    for (let i = 1; i <= 12; i++) {
