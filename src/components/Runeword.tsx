...
const tailStyle = { fontSize: 18 };           // used for the rune line tails
const HEAD_SCALE = 'text-[1.15em]';           // bump first letter size a touch

function splitRune(rune: string) {
  const head = rune.slice(0, 1);
  const tail = rune.slice(1);
  return { head, tail };
}

function renderRuneInline(rune: string) {
  if (!rune) return null;
  const { head, tail } = splitRune(rune);
  return (
    <span>
      <span className={HEAD_SCALE}>{head}</span>
      <span style={tailStyle}>{tail}</span>
    </span>
  );
}

/** Prefer array field; fall back to legacy Rune1..Rune7 */
function collectRunes(runeword: RunewordType): string[] {
  ...
  const out: string[] = [];
  for (let i = 1; i <= 7; i++) {
    const key = `Rune${i}` as keyof RunewordType;
    const v = runeword[key] as unknown as string | undefined;
    if (v) out.push(v);
  }
  return out;
}
...
function collectStats(runeword: RunewordType): LegacyStat[] {
  ...
  for (let i = 1; i <= 10; i++) {
    const k = `Stat${i}` as keyof RunewordType;
    const label = runeword[k] as unknown as string | undefined;
    if (!label) continue;
    ...
    stats.push({ code: label, min, max });
  }
  return stats;
}

/** Determine how to render a roll, handling reversed min/max gracefully. */
    ...
    >
      ...
      {runes.length > 0 && (
        <div className="text-gold whitespace-nowrap overflow-x-auto max-w-full">
          &apos;
          {runes.map((rune, i) => (
            <Fragment key={rune + i}>{renderRuneInline(rune)}</Fragment>
          ))}
          &apos;
        </div>
      )}
      ...
      <div className="mt-1 w-full grid gap-1">
        {stats.map((stat, idx) => {
          ...
          return (
            <div
              key={`${stat.code}-${idx}`}
              className="grid items-center justify-items-center gap-0.5"
            >
              {/* label */}
